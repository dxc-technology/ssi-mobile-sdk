...
class AppSocketThreadSafeProviderDuplex(
    ...
    suspend fun provideAppSocket(endpoint: String): AppSocketDuplex {
        logger.d { "AppSocketThreadSafeProvider - socket for $endpoint requested" }
        val appSocketResultDeferred = CompletableDeferred<Result<AppSocketDuplex>>()
        ...
        return when (val appSocketResult = appSocketResultDeferred.await()) {
            is Result.Success<AppSocketDuplex> -> {
    ...
    private suspend fun processRequests() {
        try {
            ...
            val appSocketsMap = sharedMutableMapOf<String, AppSocketDuplex>()
    ...
    data class GetAppSocketMessage(
        val endpoint: String,
        val appSocketResult: CompletableDeferred<Result<AppSocketDuplex>>
    ) : ControlMessage
